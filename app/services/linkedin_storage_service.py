"""
LinkedIn storage service — persist job results and list previous runs.

Every successful harvest run is saved as a timestamped JSON file under:

    data/results/linkedin/
        YYYYMMDD_HHMMSS_linkedin_jobs.json
"""
...
class LinkedInStorageService:
    ...
    def save_results(self, data: dict[str, Any]) -> str:
        ...
        filename  = f"{ts}_linkedin_jobs.json"
        ...
        logger.info("linkedin_results_saved", path=str(file_path), jobs=data.get("total_found", 0))
        return str(file_path.resolve())

    # ── Read — list ───────────────────────────────────────────────────────────

    def list_results(self) -> list[dict]:
        """Return summaries of all saved LinkedIn runs, newest first."""
        if not _BASE_PATH.exists():
            return []

        results: list[dict] = []
        for p in sorted(_BASE_PATH.glob("*_linkedin_jobs.json"), reverse=True):
            try:
                ...
                logger.debug("linkedin_result_file_skip", path=str(p), error=str(exc))
                continue
        return results
    ...
    def get_result(self, run_id: str) -> dict | None:
        ...
        if not _BASE_PATH.exists():
            return None
        for p in _BASE_PATH.glob("*_linkedin_jobs.json"):
            try:
                data = json.loads(p.read_text(encoding="utf-8"))
